...
public class AuthenticationController {
    ...
    public String esami(Model model) {
    	 try {
    	UserDetails userDetails = (UserDetails)SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    	Credentials credentials = credentialsService.getCredentials(userDetails.getUsername());
    	if (credentials.getRole().equals(Credentials.ADMIN_ROLE)) {
        ...
        return "esame/Esame";
         ...
         catch (ClassCastException e) { //l'eccezione viene sollevata se un utente non autenticato prova a caricare la voce di menu "esami", in questo caso viene mostrata la pagina relativa alla tipologia degli esami offerti
         	return "esame/Esame";
         }
    }
    
    @RequestMapping(value = "/TipologiaEsami", method = RequestMethod.GET)
    public String tipologiaEsami(Model model) {
    	 try {
    	UserDetails userDetails = (UserDetails)SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    	Credentials credentials = credentialsService.getCredentials(userDetails.getUsername());
